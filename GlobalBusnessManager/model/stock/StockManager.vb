...
Public Class StockManager
    ...
    Public Function getAllProducts() As List(Of Product)
        ...
        Try
            ...
            If mReader IsNot Nothing Then
                While mReader.Read
                    mListOfProduct.Add(New Product(CInt(mReader.GetString("id")), mReader.GetString("code"), mReader.GetString("name"), CInt(mReader.GetString("prix_achat")), CInt(mReader.GetString("prix_achat")), mReader.GetString("description"), mReader.GetString("status"), CInt(mReader.GetString("quantite"))))
                End While
            End If
        Catch ex As Exception
...
#Region "Section entrer sortir"
    ...
    Public Sub addOutObject(outObject As OutObject)
        ...
        End Try
    End Sub
    Public Sub addOutObjectByCommand(outObject As OutObject)
        mQuery = "INSERT INTO sortie (date_s, produit_id, cause, destination, quantite) VALUE(@val1, @val2, @val3, @val4, @val5)"
        Try
            mCmd = New MySqlCommand(mQuery, mConnexion)
            With mCmd.Parameters
