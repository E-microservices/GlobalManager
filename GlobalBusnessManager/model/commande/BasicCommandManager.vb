...
Public Class BasicCommandManager
    ...
    Public Sub addCommand(command As BasicCommand)
        ...
        Try
            ...
            mCmd.Parameters.AddWithValue("@val6", command.getPoduct.getPrixDeVente)
            mCmd.Parameters.AddWithValue("@val7", command.getQuantite)
            mCmd.Parameters.AddWithValue("@val8", CInt(command.getNetToPay))
            mCmd.Parameters.AddWithValue("@val9", command.getCommandStat)
...
#Region "Paralelle function et recevoir le contenu de la datagrid"
    ...
    Public Sub updateCommandDataGrid()
        mQuery = "SELECT * FROM commande ORDER BY id DESC"
        Try
            mCmd = New MySqlCommand(mQuery, mConnexion)
            mReader = mCmd.ExecuteReader
    ...
    End Function
    Public Function getRowSelectedPrixUnitaireValue() As Integer
        Dim prixUnitaire As Integer
        Try
            prixUnitaire = CInt(MainController.VU_COMMAND.CurrentRow.Cells().Item(6).Value)
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        Return prixUnitaire
    End Function
    Public Function getRowSelectedQuantiteValue() As Integer
        Dim quantite As Integer
        Try
            quantite = CInt(MainController.VU_COMMAND.CurrentRow.Cells().Item(7).Value)
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        Return quantite
    ...
    Public Function getRowSelectedNetToPayValue() As Integer
        ...
        Try
            clientPay = CInt(MainController.VU_COMMAND.CurrentRow.Cells().Item(8).Value)
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        Return clientPay
    ...
    Public Function getStatColumnCellSelectedValue() As String
        Dim forme As String
        Try
            forme = MainController.VU_COMMAND.CurrentRow.Cells().Item(9).Value
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        Return forme
    End Function
    Public Function getRowSelectedAgentValue() As String
        Dim forme As String
        Try
            forme = MainController.VU_COMMAND.CurrentRow.Cells().Item(10).Value
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        Return forme
    End Function
    Public Function getRowSelectedCommandDateValue() As String
        Dim forme As String
        Try
            forme = MainController.VU_COMMAND.CurrentRow.Cells().Item(11).Value
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        Return forme
    End Function
    Public Function getRowSelectedSourceValue() As String
        Dim forme As String
        Try
